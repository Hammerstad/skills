...
## 4. Offer the permission allowlist (ask, do not assume)

Unattended runs (`work-loop`, sub-agent reviews) stall on the first permission prompt for each new `gh` command. Offer to add the commands the workflow uses to the repo's `.claude/settings.json`, merging into the existing file rather than overwriting:
...
```json
{
  "permissions": {
    "allow": [
      ...
      "Bash(gh issue edit:*)", "Bash(gh issue comment:*)", "Bash(gh issue close:*)",
      "Bash(gh issue develop:*)",
      "Bash(gh pr list:*)", "Bash(gh pr view:*)", "Bash(gh pr diff:*)",
      "Bash(gh pr create:*)", "Bash(gh pr edit:*)", "Bash(gh pr checkout:*)",
      "Bash(gh pr checks:*)", "Bash(gh pr merge:*)",
      "Bash(gh label create:*)", "Bash(gh api:*)"
    ]
  }
}
```

Say what the broad ones grant when offering: `gh pr merge` merges without a prompt, and `gh api` can call any GitHub endpoint (the workflow needs it for review threads and sub-issues). Let the user trim the list before writing it.

## 5. Optional starter files
