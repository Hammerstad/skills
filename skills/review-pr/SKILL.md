...
For every non-trivial hunk: read the surrounding function or module at PR state, check the callers of changed signatures, check whether tests cover the changed behavior, and check whether comments, docs, or names elsewhere became stale because of this change (those become body findings). Do not run builds or tests when the repo has CI; the checks do that. When `gh pr checks` reports no checks at all, check out the PR head in a scratchpad worktree (`git worktree add --detach <scratchpad>/review-<n> FETCH_HEAD`), run the repo's build and tests there, and report a failure as a finding. Remove the worktree afterwards.
