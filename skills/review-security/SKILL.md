...
- **Sweep wide, then filter.** The checklist sweep collects every candidate; the filter step decides what ships. Never suppress a candidate while sweeping because it feels minor or probably-intentional — that judgment belongs one step later, with all of them in front of you.
- **No forced findings.** A clean scope gets "no security findings in this scope" — that is a successful review, not a failed one. This licenses an empty *result*, never a shallow sweep.
...
2. **Sweep the checklist** against the code, collecting every candidate it raises:
...
3. **Filter** the candidates: each survivor cites file:line or a config excerpt from the actual scope and states a concrete abuse case. Drop what rests on assumed infrastructure or paths you never read. A real finding you are only medium-confident about survives — say so in its confidence line rather than dropping it.
4. **Rank** by exploitability × impact, High/Medium/Low.
