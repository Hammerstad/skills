...
Then ask: what would have prevented this bug? If the answer involves a structural change (no good place for a test, tangled callers, hidden coupling), offer to file it as a `needs-grilling` issue with the specifics via `create-issue`. Make that recommendation after the fix is in rather than before, since you know more now than when you started.
